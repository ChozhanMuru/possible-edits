...
class ChiefEditorAgent:
    ...
    def __init__(self, task: dict, websocket=None, stream_output=None, tone=None, headers=None):
        ...
        # --- Add this line to initialize the parsing agent ---
    ...
    async def run_research_task(self, task_id=None):
        ...
        # After the report is generated, prompt the user for parsing
        if result and "report" in result:
            print("\n--- Final Report Generated ---\n")
            print(result["report"])
            user_choice = input("\nWould you like to parse a section of this report for data visualization? (y/n): ")
            if user_choice.strip().lower() == 'y':
                section = input("Paste the section you want to parse (or press Enter to parse the whole report): ")
                text_to_parse = section if section.strip() else result["report"]
                parsed_output = self.parsing_agent.parse(text_to_parse)
                print("\n--- Parsed Output ---")
                print(parsed_output)
                # Here you can pass parsed_output to your DataVisualizationAgent in the future
            else:
                print("Skipping parsing step.")
